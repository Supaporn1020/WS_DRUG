Public Class XML_SEARCH_DRUG_DR_TO
    'Private _XML_SEARCH_PRODUCT_GROUP As New XML_SEARCH_PRODUCT_GROUP
    'Public Property XML_SEARCH_DRUG_DR() As XML_SEARCH_PRODUCT_GROUP
    '    Get
    '        Return _XML_SEARCH_PRODUCT_GROUP
    '    End Get
    '    Set(ByVal value As XML_SEARCH_PRODUCT_GROUP)
    '        _XML_SEARCH_PRODUCT_GROUP = value
    '    End Set
    'End Property
    'Private _IDA As String
    'Public Property IDA() As String
    '    Get
    '        Return _IDA
    '    End Get
    '    Set(ByVal value As String)
    '        _IDA = value
    '    End Set
    'End Property

    'Private _pvncd As String
    'Public Property pvncd() As String
    '    Get
    '        Return _pvncd
    '    End Get
    '    Set(ByVal value As String)
    '        _pvncd = value
    '    End Set
    'End Property

    'Private _lcnsid As String
    'Public Property lcnsid() As String
    '    Get
    '        Return _lcnsid
    '    End Get
    '    Set(ByVal value As String)
    '        _lcnsid = value
    '    End Set
    'End Property

    'Private _lcnno As String
    'Public Property lcnno() As String
    '    Get
    '        Return _lcnno
    '    End Get
    '    Set(ByVal value As String)
    '        _lcnno = value
    '    End Set
    'End Property
    'Private _drgtpcd As String
    'Public Property drgtpcd() As String
    '    Get
    '        Return _drgtpcd
    '    End Get
    '    Set(ByVal value As String)
    '        _drgtpcd = value
    '    End Set
    'End Property
    'Private _rgttpcd As String
    'Public Property rgttpcd() As String
    '    Get
    '        Return _rgttpcd
    '    End Get
    '    Set(ByVal value As String)
    '        _rgttpcd = value
    '    End Set
    'End Property

    'Private _thargttpnm As String
    'Public Property thargttpnm() As String
    '    Get
    '        Return _thargttpnm
    '    End Get
    '    Set(ByVal value As String)
    '        _thargttpnm = value
    '    End Set
    'End Property

    'Private _engrgttpnm As String
    'Public Property engrgttpnm() As String
    '    Get
    '        Return _engrgttpnm
    '    End Get
    '    Set(ByVal value As String)
    '        _engrgttpnm = value
    '    End Set
    'End Property
    'Private _GROUPNAME As String
    'Public Property GROUPNAME() As String
    '    Get
    '        Return _GROUPNAME
    '    End Get
    '    Set(ByVal value As String)
    '        _GROUPNAME = value
    '    End Set
    'End Property
    'Private _phm15dgt As String
    'Public Property phm15dgt() As String
    '    Get
    '        Return _phm15dgt
    '    End Get
    '    Set(ByVal value As String)
    '        _phm15dgt = value
    '    End Set
    'End Property
    'Private _CITIZEN_AUTHORIZE As String
    'Public Property CITIZEN_AUTHORIZE() As String
    '    Get
    '        Return _CITIZEN_AUTHORIZE
    '    End Get
    '    Set(ByVal value As String)
    '        _CITIZEN_AUTHORIZE = value
    '    End Set
    'End Property
    'Private _Identify As String
    'Public Property Identify() As String
    '    Get
    '        Return _Identify
    '    End Get
    '    Set(ByVal value As String)
    '        _Identify = value
    '    End Set
    'End Property
    'Private _rgtno As String
    'Public Property rgtno() As String
    '    Get
    '        Return _rgtno
    '    End Get
    '    Set(ByVal value As String)
    '        _rgtno = value
    '    End Set
    'End Property
    'Private _lcntpcd As String
    'Public Property lcntpcd() As String
    '    Get
    '        Return _lcntpcd
    '    End Get
    '    Set(ByVal value As String)
    '        _lcntpcd = value
    '    End Set
    'End Property
    'Private _rcvno As String
    'Public Property rcvno() As String
    '    Get
    '        Return _rcvno
    '    End Get
    '    Set(ByVal value As String)
    '        _rcvno = value
    '    End Set
    'End Property
    'Private _cnccd As String
    'Public Property cnccd() As String
    '    Get
    '        Return _cnccd
    '    End Get
    '    Set(ByVal value As String)
    '        _cnccd = value
    '    End Set
    'End Property
    'Private _register As String
    'Public Property register() As String
    '    Get
    '        Return _register
    '    End Get
    '    Set(ByVal value As String)
    '        _register = value
    '    End Set
    'End Property
    'Private _register_rcvno As String
    'Public Property register_rcvno() As String
    '    Get
    '        Return _register_rcvno
    '    End Get
    '    Set(ByVal value As String)
    '        _register_rcvno = value
    '    End Set
    'End Property
    'Private _lcnno_no As String
    'Public Property lcnno_no() As String
    '    Get
    '        Return _lcnno_no
    '    End Get
    '    Set(ByVal value As String)
    '        _lcnno_no = value
    '    End Set
    'End Property
    'Private _thadrgnm As String
    'Public Property thadrgnm() As String
    '    Get
    '        Return _thadrgnm
    '    End Get
    '    Set(ByVal value As String)
    '        _thadrgnm = value
    '    End Set
    'End Property
    'Private _engdrgnm As String
    'Public Property engdrgnm() As String
    '    Get
    '        Return _engdrgnm
    '    End Get
    '    Set(ByVal value As String)
    '        _engdrgnm = value
    '    End Set
    'End Property
    'Private _drgperunit As String
    'Public Property drgperunit() As String
    '    Get
    '        Return _drgperunit
    '    End Get
    '    Set(ByVal value As String)
    '        _drgperunit = value
    '    End Set
    'End Property

    'Private _thaclassnm As String
    'Public Property thaclassnm() As String
    '    Get
    '        Return _thaclassnm
    '    End Get
    '    Set(ByVal value As String)
    '        _thaclassnm = value
    '    End Set
    'End Property


    'Private _thakindnm As String
    'Public Property thakindnm() As String
    '    Get
    '        Return _thakindnm
    '    End Get
    '    Set(ByVal value As String)
    '        _thakindnm = value
    '    End Set
    'End Property
    'Private _licen_loca As String
    'Public Property licen_loca() As String
    '    Get
    '        Return _licen_loca
    '    End Get
    '    Set(ByVal value As String)
    '        _licen_loca = value
    '    End Set
    'End Property
    'Private _fulladdr As String
    'Public Property fulladdr() As String
    '    Get
    '        Return _fulladdr
    '    End Get
    '    Set(ByVal value As String)
    '        _fulladdr = value
    '    End Set
    'End Property
    'Private _cntcd As String
    'Public Property cntcd() As String
    '    Get
    '        Return _cntcd
    '    End Get
    '    Set(ByVal value As String)
    '        _cntcd = value
    '    End Set
    'End Property
    'Private _thadsgnm As String
    'Public Property thadsgnm() As String
    '    Get
    '        Return _thadsgnm
    '    End Get
    '    Set(ByVal value As String)
    '        _thadsgnm = value
    '    End Set
    'End Property
    'Private _ctgthanm As String
    'Public Property ctgthanm() As String
    '    Get
    '        Return _ctgthanm
    '    End Get
    '    Set(ByVal value As String)
    '        _ctgthanm = value
    '    End Set
    'End Property
    'Private _dsgcd As String
    'Public Property dsgcd() As String
    '    Get
    '        Return _dsgcd
    '    End Get
    '    Set(ByVal value As String)
    '        _dsgcd = value
    '    End Set
    'End Property
    'Private _ctgcd As String
    'Public Property ctgcd() As String
    '    Get
    '        Return _ctgcd
    '    End Get
    '    Set(ByVal value As String)
    '        _ctgcd = value
    '    End Set
    'End Property
    'Private _thanm_locaion As String
    'Public Property thanm_locaion() As String
    '    Get
    '        Return _thanm_locaion
    '    End Get
    '    Set(ByVal value As String)
    '        _thanm_locaion = value
    '    End Set
    'End Property
    'Private _agent As String
    'Public Property agent() As String
    '    Get
    '        Return _agent
    '    End Get
    '    Set(ByVal value As String)
    '        _agent = value
    '    End Set
    'End Property
    'Private _cncnm As String
    'Public Property cncnm() As String
    '    Get
    '        Return _cncnm
    '    End Get
    '    Set(ByVal value As String)
    '        _cncnm = value
    '    End Set
    'End Property

    'Private _cnccsnm As String
    'Public Property cnccsnm() As String
    '    Get
    '        Return _cnccsnm
    '    End Get
    '    Set(ByVal value As String)
    '        _cnccsnm = value
    '    End Set
    'End Property
    'Private _rcvdate As String
    'Public Property rcvdate() As String
    '    Get
    '        Return _rcvdate
    '    End Get
    '    Set(ByVal value As String)
    '        _rcvdate = value
    '    End Set
    'End Property
    'Private _rcvdate_T As String
    'Public Property rcvdate_T() As String
    '    Get
    '        Return _rcvdate_T
    '    End Get
    '    Set(ByVal value As String)
    '        _rcvdate_T = value
    '    End Set
    'End Property
    'Private _appdate_T As String
    'Public Property appdate_T() As String
    '    Get
    '        Return _appdate_T
    '    End Get
    '    Set(ByVal value As String)
    '        _appdate_T = value
    '    End Set
    'End Property

    'Private _story_edit As String
    'Public Property story_edit() As String
    '    Get
    '        Return _story_edit
    '    End Get
    '    Set(ByVal value As String)
    '        _story_edit = value
    '    End Set
    'End Property
    'Private _appdate As String
    'Public Property appdate() As String
    '    Get
    '        Return _appdate
    '    End Get
    '    Set(ByVal value As String)
    '        _appdate = value
    '    End Set
    'End Property
    'Private _cncdate As String
    'Public Property cncdate() As String
    '    Get
    '        Return _cncdate
    '    End Get
    '    Set(ByVal value As String)
    '        _cncdate = value
    '    End Set
    'End Property
    'Private _appdate_th As String
    'Public Property appdate_th() As String
    '    Get
    '        Return _appdate_th
    '    End Get
    '    Set(ByVal value As String)
    '        _appdate_th = value
    '    End Set
    'End Property
    'Private _cncdate_th As String
    'Public Property cncdate_th() As String
    '    Get
    '        Return _cncdate_th
    '    End Get
    '    Set(ByVal value As String)
    '        _cncdate_th = value
    '    End Set
    'End Property
    'Private _thanm As String
    'Public Property thanm() As String
    '    Get
    '        Return _thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _thanm = value
    '    End Set
    'End Property

    'Private _cnsdnm As String
    'Public Property cnsdnm() As String
    '    Get
    '        Return _cnsdnm
    '    End Get
    '    Set(ByVal value As String)
    '        _cnsdnm = value
    '    End Set
    'End Property

    'Private _engfrgnnm As String
    'Public Property engfrgnnm() As String
    '    Get
    '        Return _engfrgnnm
    '    End Get
    '    Set(ByVal value As String)
    '        _engfrgnnm = value
    '    End Set
    'End Property
    'Private _engfrgnnm_addr As String
    'Public Property engfrgnnm_addr() As String
    '    Get
    '        Return _engfrgnnm_addr
    '    End Get
    '    Set(ByVal value As String)
    '        _engfrgnnm_addr = value
    '    End Set
    'End Property
    'Private _rid As String
    'Public Property rid() As String
    '    Get
    '        Return _rid
    '    End Get
    '    Set(ByVal value As String)
    '        _rid = value
    '    End Set
    'End Property

    'Private _cncdcd As String
    'Public Property cncdcd() As String
    '    Get
    '        Return _cncdcd
    '    End Get
    '    Set(ByVal value As String)
    '        _cncdcd = value
    '    End Set
    'End Property
    'Private _frn_no As String
    'Public Property frn_no() As String
    '    Get
    '        Return _frn_no
    '    End Get
    '    Set(ByVal value As String)
    '        _frn_no = value
    '    End Set
    'End Property
    'Private _itemno As String
    'Public Property itemno() As String
    '    Get
    '        Return _itemno
    '    End Get
    '    Set(ByVal value As String)
    '        _itemno = value
    '    End Set
    'End Property
    'Private _Ranking As String
    'Public Property Ranking() As String
    '    Get
    '        Return _Ranking
    '    End Get
    '    Set(ByVal value As String)
    '        _Ranking = value
    '    End Set
    'End Property
    'Private _typerqt As String
    'Public Property typerqt() As String
    '    Get
    '        Return _typerqt
    '    End Get
    '    Set(ByVal value As String)
    '        _typerqt = value
    '    End Set
    'End Property
    'Private _Newcode As String
    'Public Property Newcode() As String
    '    Get
    '        Return _Newcode
    '    End Get
    '    Set(ByVal value As String)
    '        _Newcode = value
    '    End Set
    'End Property
    'Private _Newcode_U As String
    'Public Property Newcode_U() As String
    '    Get
    '        Return _Newcode_U
    '    End Get
    '    Set(ByVal value As String)
    '        _Newcode_U = value
    '    End Set
    'End Property
    'Private _Newcode_R As String
    'Public Property Newcode_R() As String
    '    Get
    '        Return _Newcode_R
    '    End Get
    '    Set(ByVal value As String)
    '        _Newcode_R = value
    '    End Set
    'End Property
    'Private _Newcode_not As String
    'Public Property Newcode_not() As String
    '    Get
    '        Return _Newcode_not
    '    End Get
    '    Set(ByVal value As String)
    '        _Newcode_not = value
    '    End Set
    'End Property
    'Private _register_search As String
    'Public Property register_search() As String
    '    Get
    '        Return _register_search
    '    End Get
    '    Set(ByVal value As String)
    '        _register_search = value
    '    End Set
    'End Property
    'Private _register_search2 As String
    'Public Property register_search2() As String
    '    Get
    '        Return _register_search2
    '    End Get
    '    Set(ByVal value As String)
    '        _register_search2 = value
    '    End Set
    'End Property
    'Private _funcnm As String
    'Public Property funcnm() As String
    '    Get
    '        Return _funcnm
    '    End Get
    '    Set(ByVal value As String)
    '        _funcnm = value
    '    End Set
    'End Property
    'Private _engdrgtpnm As String
    'Public Property engdrgtpnm() As String
    '    Get
    '        Return _engdrgtpnm
    '    End Get
    '    Set(ByVal value As String)
    '        _engdrgtpnm = value
    '    End Set
    'End Property
    'Private _pvnabbr As String
    'Public Property pvnabbr() As String
    '    Get
    '        Return _pvnabbr
    '    End Get
    '    Set(ByVal value As String)
    '        _pvnabbr = value
    '    End Set
    'End Property
    'Private _lpvncd As String
    'Public Property lpvncd() As String
    '    Get
    '        Return _lpvncd
    '    End Get
    '    Set(ByVal value As String)
    '        _lpvncd = value
    '    End Set
    'End Property
    'Private _prefix_thanm As String
    'Public Property prefix_thanm() As String
    '    Get
    '        Return _prefix_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _prefix_thanm = value
    '    End Set
    'End Property
    'Private _prefix_licen As String
    'Public Property prefix_licen() As String
    '    Get
    '        Return _prefix_licen
    '    End Get
    '    Set(ByVal value As String)
    '        _prefix_licen = value
    '    End Set
    'End Property
    'Private _thaaddr_thanm As String
    'Public Property thaaddr_thanm() As String
    '    Get
    '        Return _thaaddr_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _thaaddr_thanm = value
    '    End Set
    'End Property
    'Private _tharoom_thanm As String
    'Public Property tharoom_thanm() As String
    '    Get
    '        Return _tharoom_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _tharoom_thanm = value
    '    End Set
    'End Property
    'Private _thabuilding_thanm As String
    'Public Property thabuilding_thanm() As String
    '    Get
    '        Return _thabuilding_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _thabuilding_thanm = value
    '    End Set
    'End Property
    'Private _thasoi_thanm As String
    'Public Property thasoi_thanm() As String
    '    Get
    '        Return _thasoi_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _thasoi_thanm = value
    '    End Set
    'End Property
    'Private _tharoad_thanm As String
    'Public Property tharoad_thanm() As String
    '    Get
    '        Return _tharoad_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _tharoad_thanm = value
    '    End Set
    'End Property
    'Private _thamu_thanm As String
    'Public Property thamu_thanm() As String
    '    Get
    '        Return _thamu_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _thamu_thanm = value
    '    End Set
    'End Property
    'Private _thathmblnm_thanm As String
    'Public Property thathmblnm_thanm() As String
    '    Get
    '        Return _thathmblnm_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _thathmblnm_thanm = value
    '    End Set
    'End Property
    'Private _thaamphrnm_thanm As String
    'Public Property thaamphrnm_thanm() As String
    '    Get
    '        Return _thaamphrnm_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _thaamphrnm_thanm = value
    '    End Set
    'End Property
    'Private _thachngwtnm_thanm As String
    'Public Property thachngwtnm_thanm() As String
    '    Get
    '        Return _thachngwtnm_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _thachngwtnm_thanm = value
    '    End Set
    'End Property
    'Private _zipcode_thanm As String
    'Public Property zipcode_thanm() As String
    '    Get
    '        Return _zipcode_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _zipcode_thanm = value
    '    End Set
    'End Property
    'Private _tel_thanm As String
    'Public Property tel_thanm() As String
    '    Get
    '        Return _tel_thanm
    '    End Get
    '    Set(ByVal value As String)
    '        _tel_thanm = value
    '    End Set
    'End Property
    'Private _potency As String
    'Public Property potency() As String
    '    Get
    '        Return _potency
    '    End Get
    '    Set(ByVal value As String)
    '        _potency = value
    '    End Set
    'End Property
    'Private _ExpiryDate As String
    'Public Property ExpiryDate() As String
    '    Get
    '        Return _ExpiryDate
    '    End Get
    '    Set(ByVal value As String)
    '        _ExpiryDate = value
    '    End Set
    'End Property
    'Private _expdate As String
    'Public Property expdate() As String
    '    Get
    '        Return _expdate
    '    End Get
    '    Set(ByVal value As String)
    '        _expdate = value
    '    End Set
    'End Property
    'Private _Buyers_through As String
    'Public Property Buyers_through() As String
    '    Get
    '        Return _Buyers_through
    '    End Get
    '    Set(ByVal value As String)
    '        _Buyers_through = value
    '    End Set
    'End Property
    'Private _Buyers_through_cntcd As String
    'Public Property Buyers_through_cntcd() As String
    '    Get
    '        Return _Buyers_through_cntcd
    '    End Get
    '    Set(ByVal value As String)
    '        _Buyers_through_cntcd = value
    '    End Set
    'End Property
    'Private _lmdfdate As String
    'Public Property lmdfdate() As String
    '    Get
    '        Return _lmdfdate
    '    End Get
    '    Set(ByVal value As String)
    '        _lmdfdate = value
    '    End Set
    'End Property
    'Private _indication As String
    'Public Property indication() As String
    '    Get
    '        Return _indication
    '    End Get
    '    Set(ByVal value As String)
    '        _indication = value
    '    End Set
    'End Property
End Class
